...
This paper aims to provide a comparison between the four open-source key-value stores Redis, Valkey, KeyDB and Acdis. As the main metric for this comparison, we will focus on latency. To assess their capabilities in multiple settings, we will benchmark the systems in both single-node and cluster deployments of different sizes.
...
With this study, we want to (a) examine KeyDB's claim and (b) provide users with the necessary data to choose a software solution for their deployment use cases.

// Methods

= Benchmark Design

With our benchmarks we want to give meaningful comparisons of the tested systems' latencies for the operation in real-world conditions.
...
To recreate conditions that come close to that of real-world operations, we decided to deploy both the systems under tests and the load generators to virtual machines on the Google Compute Engine.
...
#table(
    ...
    vCPUs][4][
    Memory][8 GB][
    Default egress bandwidth][Up to 10 Gbps]
]
...
A similar study was conducted by researchers at Twitter in #cite(<yangLargescaleAnalysisHundreds2021>, form: "year") @yangLargescaleAnalysisHundreds2021. In this case, the researchers analyzed the data of about 700 billion requests on 153 Twemcache instances.
...
Based on the traces collected for the general-purpose pool, the researchers provide a model to recreate realistic usage behavior for key-value stores. We will base the design of our load generator on this model.
...
In the study of Twemcache deployments at Twitter, the ratios differ from those examined at Facebook @atikogluWorkloadAnalysisLargeScale2012. While GET and SET operations are still the most commonly used, #cite(<yangLargescaleAnalysisHundreds2021>, form: "author") observed a noticeably higher average percentage of GET requests of around \~90%.
...
The first 15 values and their respective probabilities are given in @table_val_size_probs. The combined distribution is visualized in @fig_val_size_distribution.
Both distributions provide a realistic representation of object sizes, independent of temporal patterns.
...
Except for the Valkey and KeyDB single node deployments, the `GET` command consistently was the fastest of the three request types.
...
While both Valkey and KeyDB showed an increasing discrepancy in latency between the `SET` command and the `DEL` and `GET` commands for growing clusters, this behaviour could not be observed for Redis.
...
While the trend was the same for all of the three applications, the rate with which the latency decreased varies across SUTs.
...
When running the benchmarks against Acdis, we faced the issue that 
...
However, that did not change the behaviour significantly reaching the highest memory usage at $94.7%$ instead of $95.6%$.
...
Since the `SET` and `DEL` operations return output independent of the value length, our omission likely skewed the results for the `GET` command, which returns the entire value of the given key. \ \
...
In this study, we performed benchmarks on the in-memory key-value stores Redis, Valkey, KeyDB and Acdis to assess their latency for increasing cluster sizes.
We found that Redis had the lowest latency for single-node setups but had the highest latency once it was deployed as a cluster.
...
An important limitation of the benchmarking methodology for this study was the omission of key hit-rates, especially for `GET` requests. In order to provide more meaningful results, it would make sense to repeat the benchmarks described here, with a mechanism to enforce a given hit-rate.

Similarly, future work could repeat the experiments while scaling the workload with the cluster size. In this study we maintain a fixed number of requests to assess the performance improvements of adding nodes to a cluster. Since the improvements were minimal once a cluster size of three nodes was reached, scaling the number of requests could offer additional insights into the performance of the systems under test. \ \
